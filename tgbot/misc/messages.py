...
hello = (
    ...
    '/checks - список доступных проверок\n'
    '/news - новости проекта'
)

help_text = (
    'Этот бот нужен для получения уведомлений о новых проверках для тайного покупателя в БК.\n\n'
    'Как только появится доступная проверка в одном из выбранных тобой ресторанов, тебе придёт сообщение от бота.\n\n'
    'После этого ты можешь сразу открыть запись на проверку по кнопке "BurgerKing" слева от ввода сообщения.\n\n'
    'Рассылку сообщений можно выключить в профиле\n\n'
    'Доступные команды:\n'
    '/help - помощь\n'
    '/profile - ваш профиль с настройками\n'
    '/checks - список доступных проверок\n'
    '/news - новости проекта\n\n'
    'Если хотите помочь или у вас есть предложения или идеи, жмите кнопку "Связаться"'
)

news = (
    'Сейчас в боте присылаются только уведомления о новых проверках.\n\n'
    'В ближайшее время планируется добавить возможность записи на проверку по кнопке из бота, '
    'чтобы облегчить этот процесс\n\n'
    'В дальнейшем будет более глубокая интеграция с БК и, если это возможно, запись на проверку в рестораны, '
    'которые не выбраны в профиле тайного покупателя\n\n'
    'Если хотите помочь или у вас есть предложения или идеи, жмите кнопку "Связаться"'
)
...
api_error = (
    'Что-то не так с сайтом БК...\nРассылка отключена, её можно включить в профиле - /profile'
)

relogin = (
    'Повторный вход выполнен. Вы продолжите получать новые проверки'
)

bad_survey = (
    'У вас не заполнена анкета тайного покупателя!\n'
    f'Заполните анкету на {md.hlink("сайте БК", "https://burgerkingrus.ru/proverka/account/")} и повторите попытку - /start'
)
